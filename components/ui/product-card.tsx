...
// const ProductCard: React.FC<ProductCard> = ({ data, index = 0 }) => {
//   const cart = useCart()
//   // Removed unused previewModal variable
//   const router = useRouter()
//   const cardRef = useRef<HTMLDivElement>(null)
//   const isInView = useInView(cardRef, { once: true, amount: 0.2 })
...
//     >
//       {/* Image container */}
//       <div className="aspect-square w-full relative overflow-hidden bg-transparent">
//         <Image
//           src={data?.images?.[0]?.url || "/placeholder.svg"}
//           fill
//           alt={data.name || "Product image"}
...
//           {data.size?.name && <span className="text-xs uppercase text-muted-foreground">{data.size.name}</span>}
//         </div>
//       </div>
//     </motion.div>
//   )
...
const ProductCard: React.FC<ProductCard> = ({ data, index = 0 }) => {
  ...
  return (
    ...
    >
      ...
      {/* Product info */}
      <div className="mt-4 space-y-1">
        <h3 className="text-xs uppercase tracking-wider font-normal">
          {data.name}
        </h3>
        {/* <div className="flex items-center justify-between">
          <div className="akimbo-price">${Number(data?.price).toFixed(2)}</div>
